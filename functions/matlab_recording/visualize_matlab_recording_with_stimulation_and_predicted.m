function f = visualize_matlab_recording_with_stimulation_and_predicted(recording)
	...
	if isfield(recording, 'p_wave_timestamps')
        p_times = recording.p_wave_timestamps;
		for k=1:numel(p_times)
			[min_v, index] = min(abs(recording.realtime - p_times(1,k)));
			plot(recording.realtime(1, index), recording.realdata(1, index), 'b-o'); hold on;
		end
	end
